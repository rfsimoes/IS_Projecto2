...
<html>
	...
	<body>
		...
		<%  
			...
			// Lista de regiões que têm notícias escritas pelo autor
			List<String> regioes = (List<String>) session.getAttribute("regioes");
	        
	        for(int j=0; j<regioes.size(); j++){
	        	// Lista de notícias (de cada região) que são escritas pelo autor
	        	List<News> newsAuthorRegion = newsbean.newsFromAuthor(author, regioes.get(j), userdata.getUsername(), userdata.getPassword());
	        	
	        	// Se o utilizador tiver autorização para aceder ao método newsFromAuthor()
	        	if(newsAuthorRegion != null){
        ...
        %>
	        	...
	        	// Se não tiver autorização para aceder ao método newsFromAuthor()
	        	else{
	        	%>
